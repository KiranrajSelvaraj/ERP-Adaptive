...
public class ApprovalPurchaseOrderTest extends BaseClass {
	...
	@DataProvider
	public Object[][] Util1() {
		Object[][] data = Util1.getTestData("C:\\Adaptive\\Automation\\Bizapp\\ApprovalSetting.xlsx", "Sheet1");
		return data;	
	}
	...
	class PurchaseData {
		...
		public PurchaseData(String Vendor, String CurrencyCode, String CurrancyRate, String GstType, String Type,
	...
	@Test(priority = 5, dataProvider = "Util2", dependsOnMethods = "ERPLoginPage")
	public void GetData(String Vendor, String CurrencyCode, String CurrancyRate, String GstType, String Type,
		...
		PurchaseData data = new PurchaseData(Vendor, CurrencyCode, CurrancyRate, GstType, Type, ProductCode, ProductName,
	...
	//@Ignore
	@Test(priority = 9, dependsOnMethods = "ERPLoginPage")
	...
	//@Ignore
	@Test(priority = 11, dependsOnMethods = "ERPLoginPage")
	...
	public void PurchaseOrder() throws InterruptedException, IOException {
		...
		PONo = driver.findElement(By.id("PurchaseOrderNo")).getAttribute("value");
		System.out.println("PONo: "+PONo);

		int purchaseDataListSize = purchaseDataList.size();
		for (int i = 0; i < purchaseDataListSize; i++) {
			PurchaseData purchaseData = purchaseDataList.get(i);

			if (purchaseData.Vendor.isEmpty() == false) {
				Thread.sleep(2000);
				click(po.Vendor);
				click(po.VendorSearch);
				Sendkeys(po.VendorSearch, purchaseData.Vendor +Keys.ENTER);
				Thread.sleep(2000);
			...
			GstSearchInput.sendKeys(purchaseData.GstType + Keys.ENTER);

			if (i == 0) {

				getExcelGstType = purchaseData.GstType;
				getexcelOverAllDiscountType = purchaseData.OverAllDiscountType;
				getExcelOverAllDiscountPercentage = purchaseData.OverAllDiscountPercentage;
				getExcelOverAllDiscountAmount = purchaseData.OverAllDiscountAmount;
				getExcelGstPercentage = purchaseData.GstPercentage;
				getExcelCurrencyRate = purchaseData.CurrencyRate;
			}


			if (purchaseData.Type.equalsIgnoreCase("Product")) {
			...
			} else if (purchaseData.Type.equals("Service")) {

				if (!po.ServiceCheckBox.isSelected()) {
					click(po.ServiceCheckBox);
				}

			} else if (purchaseData.Type.equals("Open")) {
			...
			if (purchaseData.Type.equalsIgnoreCase("Product")) {
				...
				inputsearch.click();
				inputsearch.sendKeys(purchaseData.ProductCode +Keys.ENTER);


			} else if (purchaseData.Type.equalsIgnoreCase("Service")) {

				driver.findElement(By.xpath("//span[@id='select2-ServiceId-container']//following::input[@type='search']"))
				.sendKeys(purchaseData.ProductName + Keys.ENTER);

			} else if (purchaseData.Type.equalsIgnoreCase("Open")) {

				po.OpenProduct.sendKeys(purchaseData.ProductName + Keys.ENTER);
			...
			if (purchaseData.Type.equalsIgnoreCase("Product")) {
				...
				for (WebElement option : subUomOption) {
					if (option.getText().trim().equals(purchaseData.Uom)) {
						option.click();
						break;
					}
				}
			...
			for (product productDetails : ProductDetailsList) {
				...
				if (productDetails.productName.equalsIgnoreCase(purchaseData.ProductName)) {

					if (productDetails.IsCartonSelected.equalsIgnoreCase("true")) {

						if (purchaseData.Uom.equalsIgnoreCase("1KG") ||
								purchaseData.Uom.equalsIgnoreCase("1X10KG")||
								purchaseData.Uom.equalsIgnoreCase("1X1KG")||
								purchaseData.Uom.equalsIgnoreCase("1X1X1KG")||
								purchaseData.Uom.equalsIgnoreCase("KG")) {
			...
			//Qty:-
			Sendkeys(po.Quantity, purchaseData.Qty);
			...
			if (purchaseData.Type.equalsIgnoreCase("Product")) {
				...
				if (IsFOCManagementInPI == true) {
					...
					click(po.Foc);
					Sendkeys(po.Foc, purchaseData.Foc);
			...
			Sendkeys(po.SGD, purchaseData.Price);
			Thread.sleep(1000);
			...
			if (IsEnableItemLevelDiscountInPurchase == true) {
				...
				Sendkeys(po.DiscountAmount, purchaseData.DiscountAmount);
			...
			double Total = 0;
			double excelQtyDouble = Double.parseDouble(purchaseData.Qty);
			double priceDouble = Double.parseDouble(purchaseData.Price);

			boolean unitDiscCheckboxBoolean = Boolean.parseBoolean(purchaseData.UnitDiscCheckbox);

			if (unitDiscCheckboxBoolean == true) {

				if (purchaseData.DiscountPercentage.isEmpty() == false) {
					double discountPercentDouble = Double.parseDouble(purchaseData.DiscountPercentage);
				...
				} else if (purchaseData.DiscountAmount.isEmpty() == false) {
					double discAmtDouble = Double.parseDouble(purchaseData.DiscountAmount);
				...
				if (purchaseData.DiscountPercentage.isEmpty() == false) {
					...
					double discPercentDouble = Double.parseDouble(purchaseData.DiscountPercentage);					
					...
					double discAmtDouble1 = Double.parseDouble(purchaseData.DiscountAmount);
					double excelPriceDouble = Double.parseDouble(purchaseData.Price);					
			...
			productPrice = driver.findElement(By.xpath("//table[@id='PurchaseOrderTable']//tbody//tr//td[2]"
					+ "//div//textarea[contains(text(),'"+purchaseData.ProductName+"')]//following::td[@class='orderTotal']")).getText(); 
			...
			soft.assertEquals(productPriceDouble, expectedDiscountAmount, 
					"Actual and Expected Discount Amount Mismatched for "+purchaseData.ProductName);

			ExpSubTotal = ExpSubTotal + productPriceDouble;

			if (purchaseData.ZeroGst.equals("TRUE")) {
		...
		for (PurchaseData purchaseData : purchaseDataList) {
			...
			} else if (purchaseData.ZeroGst.equalsIgnoreCase("true") && getOverAllDiscountType.equalsIgnoreCase("%")) {
		...
		WebElement details = driver.findElement(By.xpath
				("//table[@id='POtable']//tbody//tr//td[text()='"+PONo+"']//following-sibling::td//a[@title='Edit']"));
		details.click();
		Thread.sleep(1000);
	...
	@Test(priority = 23, dependsOnMethods = "ERPLoginPage")
	public void PurchaseOrderToInvoice() throws InterruptedException {
		...
		driver.manage().timeouts().pageLoadTimeout(300, TimeUnit.SECONDS);
		driver.manage().timeouts().implicitlyWait(60, TimeUnit.SECONDS);
		System.out.println("** Purchase Order Page **");
		PurchaseOrder po = new PurchaseOrder(driver);
		...
		WebElement findOrderNo = driver.findElement(By.id("FindOrderNo"));
		findOrderNo.click();
		findOrderNo.sendKeys(PONo);		
		click(po.Fetch);

		Thread.sleep(3000);
		WebElement details = driver.findElement(By.xpath
				("//table[@id='POtable']//tbody//tr//td[text()='"+PONo+"']//following-sibling::td//a[@title='Details']"));
		details.click();

		Thread.sleep(3000);
		driver.findElement(By.xpath("//input[@value='Convert Invoice']")).click();
		System.out.println("** Purchase Order Convert Invoice **");
		...
		for (PurchaseData purchaseData : purchaseDataList) {
			...
			for (int i = 1; i <= tableSize; i++) {
				...
				if (getBatchProductName.contains(purchaseData.ProductName.trim())) {			

					if ("true".equalsIgnoreCase(purchaseData.BatchProduct)) {

						System.out.println("batch product: "+purchaseData.ProductName);

						WebElement batchFiles = driver.findElement(By.xpath
								("(//table[@id='PurchaseTable']//tbody//tr[@class='productTR'])[" + i + "]//td//textarea[contains(text(),'"+purchaseData.ProductName.trim()+"')]//following::td//a[@id='BatchFolder']"));
						js.executeScript("arguments[0].click();", batchFiles);

						String totalQty = driver.findElement(By.xpath
								("(//div//strong[contains(text(),'"+purchaseData.ProductName.trim()+"')]//following::input[@id='TotalQty'])[1]"))
								.getAttribute("value");
						System.out.println("Total Qty: "+totalQty);

						WebElement totalQtyField = driver.findElement(By.xpath
								("(//div//strong[contains(text(),'"+purchaseData.ProductName.trim()+"')]//following::input[@id='Qty'])[1]"));
						totalQtyField.click();
						...
						WebElement batchAdd = driver.findElement(By.xpath
								("(//div//strong[contains(text(),'"+purchaseData.ProductName.trim()+"')]//following::button[text()='Add'])[1]"));
						batchAdd.click();
						Thread.sleep(2000);
	...
	public void StockCalculation() {
		...
		for (int i = 0; i < purchaseDataListSize; i++) {
			...
			for (int j = 0; j < UomDetailsListSize; j++) {
				...
				if (purchaseData.Uom.equals(uomData.UomCodeValue)) {

					doubleExcelQty = Double.parseDouble(purchaseData.Qty);
			...
			for (int k = 0; k < ProductDetailsListSize; k++) {
				...
				if (productData.productName.equalsIgnoreCase(purchaseData.ProductName)) {
	...
	//@Ignore
	@Test(priority = 27, dependsOnMethods = "ERPLoginPage")
	...
	//@Ignore
	@Test(priority = 29, dependsOnMethods = "ERPLoginPage")
