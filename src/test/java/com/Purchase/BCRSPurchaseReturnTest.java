...
public class BCRSPurchaseReturnTest extends BaseClass {
	...
	private boolean IsCartonManagementInPurchase;
	...
	public void SystemSettingsPage() throws InterruptedException {
		...
		js.executeScript("arguments[0].click();", ss.SystemsSettingsParamCodeSearchField);
		ss.SystemsSettingsParamCodeSearchField.sendKeys(Keys.CONTROL + "a" + Keys.DELETE);
		Sendkeys(ss.SystemsSettingsParamCodeSearchField, "IsCartonManagementInPurchase");
		Thread.sleep(1000);
		js.executeScript("arguments[0].click();", ss.SystemSettingsFetch);
		Thread.sleep(2000);
		js.executeScript("arguments[0].click();", ss.EditSystemSetting);
		Thread.sleep(1000);
		IsCartonManagementInPurchase = ss.BooleanValue.isSelected();
		System.out.println("IsCartonManagementInPurchase :" + IsCartonManagementInPurchase);
		click(ss.Back);
	...
	//	String getExcelGstPercentage = "";
		...
		double expectedDiscountPercentage = 0;
		...
		for (int i = 0; i < excelDataListSize; i++) {
			...
			if (excelData.Vendor.isEmpty() == false) {			
				...
				click(pr.Vendor);
				WebElement vendorSearchField = driver.findElement(
						By.xpath("//span[@id='select2-VendorId-container']//following::input[@type='search']"));
				vendorSearchField.click();
				vendorSearchField.sendKeys(excelData.Vendor + Keys.ENTER); 
				Thread.sleep(2000);
				
				getGstType = driver.findElement(By.xpath("//select[@id='GSTTypeId']//child::option[@selected='selected']")).getText();
			...
			search.sendKeys(excelData.ProductCode +Keys.ENTER);
			...
			// Qty:-
			Sendkeys(pr.Qty, excelData.Qty);

			// Foc:-
			/*
			 * if (excelData.Type.equalsIgnoreCase("Product")) {
			 * 
			 * if (IsFOCManagementInPI == true) { System.out.println("IsFOCManagementInSO: "
			 * + IsFOCManagementInPI); click(pr.Foc); Sendkeys(pr.Foc, excelData.Foc);
			 * 
			 * }
			 * 
			 * } else if (excelData.Type.equalsIgnoreCase("Service")) {
			 * 
			 * click(pr.Foc); Sendkeys(pr.Foc, excelData.Foc);
			 * 
			 * } else {
			 * 
			...
			// Price:-
			click(pr.Price);
			pr.Price.sendKeys(Keys.CONTROL + "a" + Keys.DELETE);
			Sendkeys(pr.Price, excelData.Price);
			Thread.sleep(1000);

			// Item Level Discount:-
			if (IsEnableItemLevelDiscountInPurchase == true) {

				click(pr.DiscountPercentage);
				Sendkeys(pr.DiscountPercentage, Keys.CONTROL + "a" + Keys.DELETE);
				Sendkeys(pr.DiscountPercentage, excelData.DiscountPercentage);
			...
			if (excelData.BatchProduct.equalsIgnoreCase("true")) {
				...
				for (int j = 0; j < ProductDetailsList.size(); j++) {
					...
					if (product.productName.equalsIgnoreCase(excelData.ProductName)) {
						...
						if (product.IsCartonSelected.equalsIgnoreCase("true")) {
			...
			if (IsEnableBCRS == true) {
				...
				for (int j = 0; j < ProductDetailsList.size(); j++) {
					...
					if (product.productName.equalsIgnoreCase(excelData.ProductName) 
						...
						String actBcrsQty = driver.findElement(By.id("BCRSQty")).getText();
						double actBcrsQtyDouble = Double.parseDouble(actBcrsQty);
						System.out.println("Actual BCRS Qty: "+actBcrsQtyDouble);
						System.out.println();
						soft.assertEquals(actBcrsQtyDouble, runningBcrsQty, "Actual and Expected BCRS Qty Mismatched for "+excelData.ProductName);
						break;
					}
		...
		// Over All Discount:-
		click(pr.OverAllDiscountType);
		Select overAllDiscTypeSelect = new Select(pr.OverAllDiscountType);
		...
		Thread.sleep(1000);
		click(pr.OverAllDiscount);
		if (getOverAllDiscountType.equals("$")) {
			pr.OverAllDiscount.sendKeys(Keys.CONTROL + "a" + Keys.DELETE);
			Sendkeys(pr.OverAllDiscount, getExcelOverAllDiscountAmount + Keys.ENTER);
			Thread.sleep(1000);
			click(pr.Qty);

		} else if (getOverAllDiscountType.equals("%")) {
			pr.OverAllDiscount.sendKeys(Keys.CONTROL + "a" + Keys.DELETE);
			Sendkeys(pr.OverAllDiscount, getExcelOverAllDiscountPercentage + Keys.ENTER);
			Thread.sleep(1000);
			click(pr.Qty);
	...
	//	double gstPercentage = Double.parseDouble(getExcelGstPercentage);
		...
		if (getGstType.equalsIgnoreCase("Inclusive")) {
			...
			finalExpectedGstAmount = (subWithoutGstAmount * 9) / 109;
			System.out.println("Inclusive Gst Amount is: " + finalExpectedGstAmount);

		} else if (getGstType.equalsIgnoreCase("Exclusive")) {

			finalExpectedGstAmount = (subWithoutGstAmount * 9) / 100;
			System.out.println("Exclusive Gst Amount is: " + finalExpectedGstAmount);

		} else if (getGstType.equalsIgnoreCase("Zero") || getGstType.equalsIgnoreCase("Overseas")) {

			finalExpectedGstAmount = (subWithoutGstAmount * 0) / 100;
		...
		String currencyName = pr.CurrencyCode.getText().trim();
