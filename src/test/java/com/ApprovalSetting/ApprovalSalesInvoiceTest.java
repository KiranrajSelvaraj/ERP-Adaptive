...
public class ApprovalSalesInvoiceTest extends BaseClass {
	...
	@DataProvider
	public Object[][] Util1() {
		Object[][] data = Util1.getTestData("C:\\Adaptive\\Automation\\Bizapp\\ApprovalSettingSalesOrder.xlsx", "Sheet1");
		return data;	
	}
	...
	class ExcelData {
		...
		public ExcelData(String Customer, String CurrencyCode, String CurrencyRate, String GstType, String Type,
				...
				String ZeroGst, String BatchProduct) {
			super();
		...
		for (int i = 0; i < approvalDataListSize; i++) {
			...
			if (i == 0) {
				...
				/*		WebElement findElement = driver.findElement(By.xpath
						("//select[@class='filterattribute form-control select2-hidden-accessible']"));
				Select select = new Select(findElement);
				for (WebElement options : select.getOptions()) {

					if (options.getText().trim().contains(approvalData.Payment)) {
						options.click();
						break;	
					}					
				} */
	...
	public void SalesInvoice() throws InterruptedException, IOException {
		...
		String invoiceNo = driver.findElement(By.xpath
				("//table[@id='Invoicetable']//tbody//tr[1]//td[@class='sorting_2']")).getText();
		System.out.println("invoiceNo: "+invoiceNo);

		String invoiceStatus = driver.findElement(By.xpath
				("//table[@id='Invoicetable']//tbody//tr[1]//td[text()='"+invoiceNo+"']//following-sibling::td//span[@class='label label-warning']")).getText();
