...
public class StockAdjustmentV2Test extends BaseClass{
	...
	public void StockAdjustment() {
		...
		wait.until(ExpectedConditions.visibilityOf(sa.CreateV2)).click();
		...
		for (int i = 0; i < ExcelDataListSize; i++) {
			...
			if ("true".equalsIgnoreCase(excelData.BatchProduct)) {
				...
				WebElement close = driver.findElement(By.xpath("(//label[text()='"+excelData.ProductName+"']//preceding::span[@class='tooltipclose'])["+i+"]"));
