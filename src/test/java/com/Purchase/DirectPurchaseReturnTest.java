...
import com.PomClass.PurchaseReturns;
...
public class DirectPurchaseReturnTest extends BaseClass {
	...
	@DataProvider
	public Object[][] Util2() {
		Object[][] data = Util1.getTestData("C:\\Adaptive\\Automation\\Bizapp\\DirectPurchaseReturn.xlsx", "Sheet1");
		return data;
	...
	class ExcelData {
		...
		public ExcelData(String Vendor, String CurrencyCode, String CurrancyRate, String GstType, String Type,
				...
				String BatchProduct, String BatchNo, String ReturnQty) {
			super();
	...
	public void PurchaseReturn() throws InterruptedException, IOException {
		...
		/*	List<WebElement> prodOptions = driver.findElements(
			        By.xpath("//ul[@id='select2-ProductId-results']//li"));

			for (WebElement option : prodOptions) {
			    String fullText = option.getText().trim();
			    if (!fullText.contains("-")) {
			        continue; // safety check
			    }
			    String prodCode = fullText.split(" - ")[0].trim();
			    if (prodCode.equalsIgnoreCase(excelData.ProductCode.trim())) {
			        option.click();
			        break;
			    }
			} */
			...
			if (excelData.BatchProduct.equalsIgnoreCase("true")) {
				...
				Thread.sleep(2000);
				String totalQty = driver.findElement(By.xpath("(//div//strong[contains(text(),'" + excelData.ProductName
						+ "')]//following::input[@id='TotalQty'])[1]")).getAttribute("value");
