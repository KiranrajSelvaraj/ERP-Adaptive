...
public class BCRSSalesInvoiceTest extends BaseClass {
	...
	private boolean IsEnableBCRS;
	private String BCRSDepositServiceCode;
	...
	public void SystemSettings() throws InterruptedException {
		...
		Sendkeys(ss.SystemsSettingsParamCodeSearchField, "Enable BCRS");
		Thread.sleep(1000);
		js.executeScript("arguments[0].click();", ss.SystemSettingsFetch);
		Thread.sleep(2000);
		js.executeScript("arguments[0].click();", ss.EditSystemSetting);
		Thread.sleep(1000);
		IsEnableBCRS = ss.BooleanValue.isSelected();
		System.out.println("IsEnableBCRS :" + IsEnableBCRS);
		click(ss.Back);

		Thread.sleep(2000);
		js.executeScript("arguments[0].click();", ss.SystemsSettingsParamCodeSearchField);
		ss.SystemsSettingsParamCodeSearchField.sendKeys(Keys.CONTROL + "a" + Keys.DELETE);
		Sendkeys(ss.SystemsSettingsParamCodeSearchField, "BCRSDepositServiceCode");
		Thread.sleep(1000);
		js.executeScript("arguments[0].click();", ss.SystemSettingsFetch);
		Thread.sleep(2000);
		js.executeScript("arguments[0].click();", ss.EditSystemSetting);
		Thread.sleep(1000);
		BCRSDepositServiceCode = driver.findElement(By.id("StringValue")).getAttribute("value");
		System.out.println("BCRSDepositServiceCode :" + BCRSDepositServiceCode);
		click(ss.Back);
	...
	class product {
		...
		public product(String productCode, String productName, String departmentValue, String categoryValue,
				...
				String batch, String bcrsProduct, String purchaseCOAValue, String salesCOAValue, String currentStockValue,
	...
	public void ProductPage() throws InterruptedException {
		...
		for (String product : ProductSet) {
			...
			String bcrsProduct = driver.findElement(By.xpath("(//dt[text()='BCRS Product']//following-sibling::dd)[1]"))
					.getText();
			System.out.println("BCRS Product: "+bcrsProduct);
			...
			product pr = new product(productCode, productName, departmentValue, categoryValue, brandValue,
					profitMarginValue, marginToleranceValue, vendorNameValue, batch, bcrsProduct, purchaseCOAValue, salesCOAValue,
	...
	public void ServicePage() throws InterruptedException {
		...
		int serviceTableSize = driver.findElements(By.xpath("//table[@id='servicetable']//tbody//tr"))
				.size();
		for (int i = 0; i < serviceTableSize; i++) {

			Thread.sleep(2000);
			WebElement detailsIcon = driver
					.findElement(By.xpath("//table[@Id='servicetable']//tbody//tr//td//child::a[@title='Details']"));
			detailsIcon.click();
			Thread.sleep(3000);
	...
	public void SalesInvoice() throws InterruptedException, IOException {
		...
		double runningBcrsQty = 0;
		double runningBcrsAmount = 0;
		...
		for (int i = 0; i < excelDataListSize; i++) {	
			...
			if (IsEnableBCRS == true) {

				for (int j = 0; j < ProductDetailsList.size(); j++) {
					product product = ProductDetailsList.get(j);

					if (product.productName.equalsIgnoreCase(excelData.ProductName)
							&& product.bcrsProduct.equalsIgnoreCase("true")) {

						double multipleQty = 0;
						double amount = 0;

						for (int k = 0; k < UomDetailsList.size(); k++) {
							UOM uom = UomDetailsList.get(k);

							if (uom.UomCodeValue.equals(excelData.Uom)) {

								double uomUnitDouble = Double.parseDouble(uom.UomUnits);
								multipleQty = uomUnitDouble * qtyDouble;
								break;
							}
						}
						...
						if (!serviceDetailsList.isEmpty()) {
							...
							service service = serviceDetailsList.get(0);

							double unitPriceDouble = Double.parseDouble(service.unitPrice);
							amount = multipleQty * unitPriceDouble;
						}

						runningBcrsQty += multipleQty;
						runningBcrsAmount += amount;

						String bcrsQty = driver.findElement(By.xpath("//table[@id='SalesTable']//tbody//tr//td//textarea[text()='Z Delivery Charges']//following::td//input[@id='ItemLQty']"))
								.getAttribute("value");
						double actBcrsQty = Double.parseDouble(bcrsQty);
						System.out.println("Actual BCRS Qty: "+actBcrsQty);

						System.out.println("Expected BCRS Qty: " + runningBcrsQty);
						System.out.println();
						System.out.println("Expected BCRS Amount: " + runningBcrsAmount);

						String bcrsAmount = driver.findElement(By.xpath("//table[@id='SalesTable']//tbody//tr//td//textarea[text()='Z Delivery Charges']//following::td//p[@class='totaldetailamount']"))
								.getText();
						double actBcrsAmount = Double.parseDouble(bcrsAmount);
						System.out.println("Actual BCRS Amount: "+actBcrsAmount);
		...
		double bcrsExpSubTotal = expSubtotal + runningBcrsAmount;
