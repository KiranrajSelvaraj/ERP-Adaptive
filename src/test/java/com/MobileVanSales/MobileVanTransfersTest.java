...
public class MobileVanTransfersTest extends BaseClass {
	...
	public Object[][] Util1() {
		...
		Object data[][] = Util1.getTestData("C:\\Adaptive\\Automation\\Bizapp\\MobileVanTransfers.xlsx", "Sheet1");
		return data;
	...
	class ExcelData {
		...
		public ExcelData(String FromWarehouse, String ToMobileVan, String ProductCode, String ProductName,
				String Uom, String Qty, String Department, String BatchProduct) {
			super();
	...
	public void GetData(String FromWarehouse, String ToMobileVan, String ProductCode, String ProductName,
			String Uom, String Qty, String Department, String BatchProduct) {

		ExcelData data = new ExcelData(FromWarehouse, ToMobileVan, ProductCode, ProductName, Uom, Qty, Department, BatchProduct);
	...
	public void MobileVanTransfers() throws InterruptedException {
		...
		String getExcelToMobileVan = "";

		//	click(mvt.CreateNew);
		//	wait.until(ExpectedConditions.visibilityOf(mvt.CreateNew)).click();
		js.executeScript("arguments[0].click();", mvt.CreateNew);

		int excelDataListSize = excelDataList.size();
		for (int i = 0; i < excelDataListSize; i++) {
			ExcelData excelData = excelDataList.get(i);

			String getFromWarehouse = driver.findElement(By.xpath
					("//span[@id='select2-FromWarehouseId-container']")).getAttribute("title");
			System.out.println("getFromWarehouse: "+getFromWarehouse);

			String getToMobileVan = driver.findElement(By.xpath
					("//span[@id='select2-ToMobileVanId-container']")).getAttribute("title");
			System.out.println("getToMobileVan: "+getToMobileVan);

			if (i == 0) {

				getExcelFromWarehouse = excelData.FromWarehouse;
				getExcelToMobileVan = excelData.ToMobileVan;

			}

			if (!getExcelFromWarehouse.equalsIgnoreCase(getFromWarehouse)) {

				click(mvt.FromWarehouse);
			...
			if (!getExcelToMobileVan.equalsIgnoreCase(getToMobileVan)) {

				click(mvt.ToMobileVan);
				WebElement toMobileVanSearch = driver.findElement(By.xpath
						("//span[@id='select2-ToMobileVanId-container']//following::input[@type='search']"));
				toMobileVanSearch.sendKeys(excelData.ToMobileVan +Keys.ENTER);
			...
			if (i <= 15) {
				...
				WebElement productSearch = driver.findElement(By.xpath
						("//span[@id='select2-Product-container']//following::input[@type='search']"));
				...
				click(mvt.Qty);
				Sendkeys(mvt.Qty, excelData.Qty);
				...
				if (i > 15) {
					...
					Thread.sleep(2000);
					click(mvt.AddItemsDepartment);
					WebElement departmentSearchField = driver.findElement(By.xpath("//span[@id='select2-DepartmentId-container']//following::input[@type='search']"));
					departmentSearchField.sendKeys(excelData.Department +Keys.ENTER);

					WebElement addItemSearchBox = driver.findElement(By.xpath("//div//child::input[@placeholder='Search...']"));
					addItemSearchBox.click();
					addItemSearchBox.sendKeys(excelData.ProductCode);

					WebElement lQtyBox = driver.findElement(By.xpath
							("(//table[@id='show-product-table']//tbody//tr//td[contains(text(),'"+excelData.ProductCode+"')]//following::td[contains(text(),'"+excelData.Uom+"')]//following::input[@id='LQty'])[1]"));
					lQtyBox.click();
					lQtyBox.sendKeys(excelData.Qty);

					click(mvt.AddItemsAdd);

					if (excelData.BatchProduct.equalsIgnoreCase("true")) {

						WebElement addItemBatchFile = driver.findElement(By.xpath
								("//table[@id='tb_products']//tr//td[contains(text(),'"+excelData.ProductName.trim()+"')]//following-sibling::td[6]//a[@class='fa fa-folder-open Popup']"));
						addItemBatchFile.click();
